...
export class Engine {
    private orderbooks: Orderbook[] = []; //the engine will manage multiple order books
    ...
    //process function check what type of order is coming from the API
    process({ message, clientId }: {message: MessageFromApi, clientId: string}) {
        switch (message.type) {
            //create an order
            case CREATE_ORDER:
                try {
                    //createOrder function returns how much order is executed, how many fills happened
                    const { executedQty, fills, orderId } = this.createOrder(message.data.market, message.data.price, message.data.quantity, message.data.side, message.data.userId);
                    
                    //engine publish into pubsub
            ...
            case CANCEL_ORDER:
                ...
                break;
            // if user want to see all of their currently present orders
            ...
            case ON_RAMP:
                ...
                break;
            //when user wants to see the current depth of the orderbook    
            case GET_DEPTH:
                try {
    ...
    createOrder(market: string, price: string, quantity: string, side: "buy" | "sell", userId: string) {
        //get orderbook of the specific market
        const orderbook = this.orderbooks.find(o => o.ticker() === market)
        ...
        //before even placing order, we need to check and lock the user's funds
        this.checkAndLockFunds(baseAsset, quoteAsset, side, userId, quoteAsset, price, quantity);  //pre balance check
        ...
        this.updateBalance(userId, baseAsset, quoteAsset, side, fills, executedQty);  //post balance check
    ...
    checkAndLockFunds(baseAsset: string, quoteAsset: string, side: "buy" | "sell", userId: string, asset: string, price: string, quantity: string) {
        //or you try to buy or sell
        if (side === "buy") {
            //check if user has enough funds to buy specified quantity
            if ((this.balances.get(userId)?.[quoteAsset]?.available || 0) < Number(quantity) * Number(price)) {
                throw new Error("Insufficient funds");
            }
            //available usdc goes down
            //@ts-ignore
