...
//every order you store must have how much quantity is filled(filled)
...
export class Orderbook {
    ...
    matchBid(order: Order): {fills: Fill[], executedQty: number} {
        ...
        for (let i = 0; i < this.asks.length; i++) {
            //if user order is completely filled then can break from the loop
            if (executedQty == order.quantity) {
                break;
            }
            if (this.asks[i].price <= order.price) {
                ...
                //if any fill happen push it fills array (fills array is what all quantities were able to be filled by the current orderbook)
                fills.push({
        ...
        //if ask's filled quantity is equal to the quantity the user wanted to buy or sell then can remove from orderbook
        for (let i = 0; i < this.asks.length; i++) {
